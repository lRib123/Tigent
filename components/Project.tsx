...
const dashboard = () => {navigation.navigate('Dashboard')}
  ...
  useEffect(() => {
    const getSavedData = async () => {
      const savedInputData = await AsyncStorage.getItem('project');
  ...
  useEffect(() => {
    const saveData = async () => {
      await AsyncStorage.setItem('project', JSON.stringify(inputData));
    };
    saveData();
  }, [inputData]);
  ...
  const handleAddIdea = async () => {
    ...
    setIdea(updatedIdea);
    await AsyncStorage.setItem('project', JSON.stringify(updatedIdea));
  };
