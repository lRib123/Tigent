...
const Idea = () => {
  ...
  const [incompleted2,setinCompleted2] = useState(true);
  const [progress2,setProgress2] = useState(true);
  const [mode,setMode] = useState('time')
  useEffect(() => {
    const getSavedData = async () => {
      const savedInputData = await AsyncStorage.getItem('idea');
  ...
  useEffect(() => {
    const saveData = async () => {
      await AsyncStorage.setItem('idea', JSON.stringify(inputData));
    };
    saveData();
  }, [inputData]);
  ...
  const handleAddIdea = async () => {
    ...
    setIdea(updatedIdea);
    await AsyncStorage.setItem('idea', JSON.stringify(updatedIdea));
  };
